...
if ($dbflag==1)
  {
  $server= "db93.puretec.de"; // Adresse des Datenbankservers
  $user= "p33675770";         // FTP-Benutzername
  $passwort= "testdat1";        // Passwort
  $datenbank= "db65775458";   // Name der Datenbank
  }
else
  {
...
$datenbank= "mwerkcrm";
//  $server= "localhost"; // Adresse des Datenbankservers
//  $user= "biorgrou";         // FTP-Benutzername
//  $passwort= "auVeuthEE";        // Passwort
//  $datenbank= "biorgrou";   // Name der Datenbank
  }
...
function add_vchistory($id)
  ...
  MYSQL_QUERY("insert into dslprov_vchistory (vc_id,vp_id,btproduct_id,vispproduct_id,vc_cli,userbw_id,usercr_id,userrealm_id,visp_id,domain1_id,domain2_id,domain3_id,vc_carelevel,vc_state,vc_statechangedate) select vc_id,vp_id,btproduct_id,vispproduct_id,vc_cli,userbw_id,usercr_id,userrealm_id,visp_id,domain1_id,domain2_id,domain3_id,vc_carelevel,vc_state,vc_statechangedate from dslprov_vc where vc_id='$id'");
  }
...
function get_usercr($id)
  {
  $query="select usercr_value from dslprov_usercr where usercr_id='$id'";
  $result=mysql_query($query);
  $number=mysql_num_rows($result);
  if ($number!=1)
    {
  ...
  $ArticleTextRow=mysql_fetch_array($result);
  $value=$ArticleTextRow['usercr_value'];
  return($value);
  }


function get_userrealm($id)
  {
  $query="select userrealm_value from dslprov_userrealm where userrealm_id='$id'";
  $result=mysql_query($query);
  $number=mysql_num_rows($result);
  if ($number!=1)
    {
  ...
  $ArticleTextRow=mysql_fetch_array($result);
  $value=$ArticleTextRow['userrealm_value'];
  return($value);
  }
...
function get_btproduct($id)
  {
  $query="select btproduct_value from dslprov_btproduct where btproduct_id='$id'";
  $result=mysql_query($query);
  $number=mysql_num_rows($result);
  if ($number!=1)
    {
