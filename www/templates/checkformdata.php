...
$serdatum=get_sertime("2019-01-10");
print $serdatum;
...
$errormessagetwo="Falscher Eintrag in ";
      $ordererror=0;
      $xordererror=0; // fuer die OE ERRORS
      $oecode=0;
...
#echo ("OE $ordererror "); 
      ...
      if ((strlen($vc_contact_email)>0))
        {
        if(checktext($vc_contact_email,"email Kontakt Adresse",13)) {$ordererror++; $c_vc_contact_email=$alertcolor;}
        }
...
echo ("OE $ordererror ");
      ...
      if(checktext($vc_tbfirstname,"TB Vorname",2302)) {$ordererror++; $c_vc_tbfirstname=$alertcolor;}
      if(checktext($vc_tbname,"TB Nachname",2302)) {$ordererror++; $c_vc_tbname=$alertcolor;}
...
$checkiban=-1;
if ($vc_billingtype=='DEBIT')
  {
      // BANK

      $oecodebank=0;
      
      if(checktext($vc_bankname,"Geldinstitut",1009)) {$xordererror++;$oecodebank++; $c_vc_bankname=$alertcolor;}
      if(checktext($vc_bankaccholder,"Kontoinhaber",1001)) {$xordererror++;$oecodebank++; $c_vc_bankaccholder=$alertcolor;}

echo("OEBANK $oecodebank");

//NEU 11.18
if(checktext($vc_bankaccholderpre,"Kontoinhaber Vorname",1001)) {$xordererror++;$oecodebank++; $c_vc_bankaccholderpre=$alertcolor;}
if(checktext($vc_bankaccholderstr,"Kontoinhaber Strasse",2)) {$xordererror++;$oecodebank++; $c_vc_bankaccholderstr=$alertcolor;}
if(checktext($vc_bankaccholdernr,"Kontoinhaber Hausnummer",3100)) {$xordererror++;$oecodebank++; $c_vc_bankaccholdernr=$alertcolor;}
if(checktext($vc_bankaccholderzip,"Kontoinhaber PLZ",3001)) {$xordererror++;$oecodebank++; $c_vc_bankaccholderzip=$alertcolor;}
if(checktext($vc_bankaccholdercity,"Kontoinhaber Ort",2)) {$xordererror++;$oecodebank++; $c_vc_bankaccholdercity=$alertcolor;}
      ...
      if (((strlen($vc_bankiban)>0))&&((strlen($vc_bankaccnumber)==0)&&(strlen($vc_banksortcode)==0)))
        {
        $checkiban=1;
        if(checktext($vc_bankiban,"IBAN",8)) {$xordererror++;$oecodebank++; $c_vc_bankiban=$alertcolor;}
        }
      else
      //if ((strlen($vc_bankaccnumber)>0)||(strlen($vc_banksortcode)>0))
        {
        if(checktext($vc_bankaccnumber,"Kontonummer",4)) {$xordererror++;$oecodebank++; $c_vc_bankaccnumber=$alertcolor;}
        if(checktext($vc_banksortcode,"Bankleitzahl",5)) {$xordererror++;$oecodebank++; $c_vc_banksortcode=$alertcolor;}
        }
...
$oecodegee=0;
if(checktext($gee_owner_name,"GEE Eigent&uuml;mer",1)) {$xordererror++;$oecodegee++; $c_gee_owner_name=$alertcolor;}
if(checktext($gee_contact_company,"GEE Kontakt Firma",2)) {$xordererror++;$oecodegee++; $c_gee_contact_company=$alertcolor;}
if(checktext($gee_contact_name,"GEE Kontakt Name",2)) {$xordererror++;$oecodegee++; $c_gee_contact_name=$alertcolor;}
if(checktext($gee_contact_str,"GEE Kontakt Strasse Hausnummer",22)) {$xordererror++;$oecodegee++; $c_gee_contact_str=$alertcolor;}
if(checktext($gee_contact_zip,"GEE Kontakt PLZ",3001)) {$xordererror++;$oecodegee++; $c_gee_contact_zip=$alertcolor;}
if(checktext($gee_contact_city,"GEE Kontakt Ort",2)) {$xordererror++;$oecodegee++; $c_gee_contact_city=$alertcolor;}
if(checktext($gee_contact_tel,"GEE Kontakt Telefon",9001)) {$xordererror++;$oecodegee++; $c_gee_contact_tel=$alertcolor;}
if(checktext($gee_contact_email,"GEE Kontakt Email",1301)) {$xordererror++;$oecodegee++; $c_gee_contact_email=$alertcolor;}
if(checktext($gee_flur,"GEE Flur",2302)) {$xordererror++;$oecodegee++; $c_gee_flur=$alertcolor;}

// GEE kein Fehler mehr auskommentiert am 19.05.2020
// if ($oecodegee>0) {$oecode+=4096;}

      // GEE FILE

$oecodegeefile=0;
if(checktext($vc_docname_gee,"Filename GEE",2301)) {$xordererror++;$oecodegeefile++; $c_vc_docname_gee=$alertcolor;}
...
if(checktext($vc_docname_contract,"Filename Vertrag",2301)) {$xordererror++;$oecodecontractfile++; $c_vc_docname_contract=$alertcolor;}

// auskommentiert 20220303
...
if(checktext($foreign_customer_id,"vorh. Kundennummer",22)) {$ordererror++;$c_foreign_customer_id=$alertcolor;}
...
// SELECT vc_visprefnum,vc_errorcode,b.vcfault_id,b.vcfault_text from dslprov_vc a,dslprov_vcfault b where vc_errorcode>0 and (b.vcfault_id & a.vc_errorcode) order by vc_visprefnum,vcfault_id;
   ...
   if ((strlen($portprovider_id)>0)||(strlen($vc_port_onkz)>0)||(strlen($vc_port_n1)>0)||(strlen($vc_port_n2)>0)||(strlen($vc_port_n3)>0)||(strlen($vc_port_n4)>0)||(strlen($vc_port_n5)>0)||(strlen($vc_port_n6)>0)||(strlen($vc_port_n7)>0)||(strlen($vc_port_n8)>0)||(strlen($vc_port_n9)>0)||(strlen($vc_port_n10)>0))
      {
        //echo("PORT   ");
           ...
           // AWA FILE
// hier logisch reingenommen 20181221 evtl. muss das frei stehen.
     $oecodeawafile=0;
     if(checktext($vc_docname_awa,"Filename AWA",2301)) {$xordererror++;$oecodeawafile++; $c_vc_docname_awa=$alertcolor;}
...
//     if ($oecodeawafile>0) {$oecode+=512;}
      ...
      if(checktext($portprovider_id,"Port Provider",21)) {$xordererror++; $oecodeawa++;  $c_vc_port=$alertcolor;}
      if(checktext($vc_port_onkz,"ONKZ",9)) {$xordererror++; $oecodeawa++; $c_vc_port=$alertcolor;}
      if(checktext($vc_port_n1,"N1",14)) {$xordererror++; $oecodeawa++; $c_vc_port=$alertcolor;}      
      if(checktext($vc_port_n2,"N2",16)) {$xordererror++; $oecodeawa++; $c_vc_port=$alertcolor;}      
      if(checktext($vc_port_n3,"N3",16)) {$xordererror++; $oecodeawa++; $c_vc_port=$alertcolor;}      
      if(checktext($vc_port_n4,"N4",16)) {$xordererror++; $oecodeawa++; $c_vc_port=$alertcolor;}      
      if(checktext($vc_port_n5,"N5",16)) {$xordererror++; $oecodeawa++; $c_vc_port=$alertcolor;}      
      if(checktext($vc_port_n6,"N6",16)) {$xordererror++; $oecodeawa++; $c_vc_port=$alertcolor;}      
      if(checktext($vc_port_n7,"N7",16)) {$xordererror++; $oecodeawa++; $c_vc_port=$alertcolor;}      
      if(checktext($vc_port_n8,"N8",16)) {$xordererror++; $oecodeawa++; $c_vc_port=$alertcolor;}      
      if(checktext($vc_port_n9,"N9",16)) {$xordererror++; $oecodeawa++; $c_vc_port=$alertcolor;}      
      if(checktext($vc_port_n10,"N10",16)) {$xordererror++; $oecodeawa++; $c_vc_port=$alertcolor;}      
...
echo("OECODE: $oecode ORDERERR $ordererror ");

//if ($vc_state>21)
if ($vc_state!=20)
  {
  $ordererror=$ordererror+$xordererror;
  }

echo("CHECKFORMDATA VCSTATE $vc_state ORDERERROR $ordererror CIBAN $checkiban");
...
if ($debug)  print"<pre> IBANCHECKER<br>";
if ($debug)  print_r($result);
if ($debug)  print"<br>IBANCHECKER</pre>";
...
if ($debug)  print"</pre>";
  ...
  if ($resresult=='passed') 
    {
    print "$checkiban $resiban $resresult $resreturncode<br>";
    }                                                    
  else
    {
    $ordererror=1;
...
function checktext($text,$descr,$mode)
  ...
  if ($mode==1)
    {
    //if(eregi("^[_0-9a-zA-z‰ˆ¸ƒ÷‹ﬂ.\ ]{2,30}$",$text)==0)
...
<b>$descr</b></font><br>");
  ...
  if ($mode==2)
    {
    //if(eregi("^[_0-9a-zA-z‰ˆ¸ƒ÷‹ﬂ.\ ]{0,30}$",$text)==0)     // UMLAUTE FEHLEN
...
<b>$descr</b></font><br>");
  ...
  if ($mode==2301)
    {
    if(preg_match ("/^[0-9a-zA-Z\x7f-\xff.,\ \-\_]{2,30}$/",$text)==0)    // UMLAUTE FEHLEN
      {
      $error=1;
if ($debug)      echo("<font color=red>$errormessage
<b>$descr</b></font><br>");
  ...
  if ($mode==2302)
    {
    if(preg_match ("/^[0-9a-zA-Z\x7f-\xff.,\ \-\_]{0,30}$/",$text)==0)    // UMLAUTE FEHLEN
      {
      $error=1;
if ($debug)      echo("<font color=red>$errormessage
<b>$descr</b></font><br>");
...
//    if (preg_match('/^[^0-9][a-zA-Z0-9-_.]+([.][a-zA-Z0-9-_]+)*[@][a-zA-Z0-9-_]+([.][a-zA-Z0-9-_]+)*[.][a-zA-Z]{2,10}$/', $text)) 
    if (preg_match('/^[a-zA-Z0-9-_.]+([.][a-zA-Z0-9-_]+)*[@][a-zA-Z0-9-_]+([.][a-zA-Z0-9-_]+)*[.][a-zA-Z]{2,10}$/', $text)) 
      {
      //OK
      }  
    else
...
//    if ((preg_match('/^[^0-9][a-zA-Z0-9-_.]+([.][a-zA-Z0-9-_]+)*[@][a-zA-Z0-9-_]+([.][a-zA-Z0-9-_]+)*[.][a-zA-Z]{2,10}$/', $text))||((strlen($text)==0))) 
    if ((preg_match('/^[a-zA-Z0-9-_.]+([.][a-zA-Z0-9-_]+)*[@][a-zA-Z0-9-_]+([.][a-zA-Z0-9-_]+)*[.][a-zA-Z]{2,10}$/', $text))||((strlen($text)==0))) 
      {
      //OK
      }  
    else
