...
//print "XXX $datenbank CISP $cisp ISP $isp";
...
if (strlen($changeinfo)>0)
  {
  // DB
  MYSQL_QUERY("UPDATE `dslprov_vc` SET vc_confirmations=vc_confirmations+1 WHERE vc_visprefnum=$changeinfo AND !(1 & vc_confirmations);");
  }
  
$changeab = ( ! empty( $_GET[ 'changeab' ] ) ) ? $_GET[ 'changeab' ] : false;
if (strlen($changeab)>0)
  {
  // DB
  MYSQL_QUERY("UPDATE `dslprov_vc` SET vc_confirmations=vc_confirmations+2 WHERE vc_visprefnum=$changeab AND !(2 & vc_confirmations);");
  MYSQL_QUERY("INSERT INTO `dslprov_vc_confirmations` (conf_number,conf_visprefnum,conf_state,conf_type,conf_date,conf_billoid) VALUES ('$changeab', '$changeab', 'DOKREQUEST', 'AB', '', '0');");
  $changeab='';
  }


$resetab = ( ! empty( $_GET[ 'resetab' ] ) ) ? $_GET[ 'resetab' ] : false;
if (strlen($resetab)>0)
  {
  // DB
  MYSQL_QUERY("UPDATE `dslprov_vc` SET vc_confirmations=vc_confirmations-2 WHERE vc_visprefnum=$resetab AND (2 & vc_confirmations);");
  $resetab='';
  }
...
$changeeb = ( ! empty( $_GET[ 'changeeb' ] ) ) ? $_GET[ 'changeeb' ] : false;
if (strlen($changeeb)>0)
  {
  // DB
//  MYSQL_QUERY("UPDATE `dslprov_vc` SET vc_confirmations=vc_confirmations+4 WHERE vc_visprefnum=$changeeb AND !(4 & vc_confirmations);");
  }
...
if (strlen($resetstate1)>0)
  {
  // DB
  MYSQL_QUERY("UPDATE `dslprov_vc` SET vc_state=22,vc_state_voice=22,vc_confirmations=0,vc_salesreported=0,vc_cbuk='REPROV1' WHERE vc_visprefnum=$resetstate1;");
  MYSQL_QUERY("delete FROM dslprov_vc_subscribernumbers WHERE subscriber_visprefnum=$resetstate1;");
  MYSQL_QUERY("delete FROM dslprov_vc_line WHERE line_number=$resetstate1;");
  }
$resetstate3 = ( ! empty( $_GET[ 'resetstate3' ] ) ) ? $_GET[ 'resetstate3' ] : false;
if (strlen($resetstate3)>0)
  {
  // DB
  MYSQL_QUERY("UPDATE `dslprov_vc` SET vc_state=22,vc_state_voice=22,vc_confirmations=0,vc_salesreported=0,vc_cbuk='REPROV3' WHERE vc_visprefnum=$resetstate3;");
  MYSQL_QUERY("delete FROM dslprov_vc_subscribernumbers WHERE subscriber_visprefnum=$resetstate3;");
  MYSQL_QUERY("delete FROM dslprov_vc_line WHERE line_number=$resetstate3;");
  }


$visp_id = ( ! empty( $_GET[ 'visp_id' ] ) ) ? $_GET[ 'visp_id' ] : false;
$vc_migkey = ( ! empty( $_GET[ 'vc_migkey' ] ) ) ? $_GET[ 'vc_migkey' ] : false;
...
$setrdddate=$strddparts[0].'.'.$strddparts[1].'.'.$strddparts[2];

echo("$setrdddate");
$ser_rdd=get_sertime($setrdddate);
echo("$ser_rdd");
...
if (strlen($ser_rdd)>0)
  ...
  echo("RDD $setrdd");
  // DB
  MYSQL_QUERY("UPDATE `dslprov_vc` SET vc_state=22,vc_state_voice=22,asap_id=0,vc_reqdeliverydate='$ser_rdd',vc_reqdeliverydate_voice='$ser_rdd' WHERE vc_visprefnum=$setrdd;");
  }
...
$vc_id = ( ! empty( $_GET[ 'vc_id' ] ) ) ? $_GET[ 'vc_id' ] : false;
...
$setcanceldate = ( ! empty( $_GET[ 'setcanceldate' ] ) ) ? $_GET[ 'setcanceldate' ] : false;
$setpchangedate = ( ! empty( $_GET[ 'setpchangedate' ] ) ) ? $_GET[ 'setpchangedate' ] : false;
$product_id_change = ( ! empty( $_GET[ 'product_id_change' ] ) ) ? $_GET[ 'product_id_change' ] : false;
...
$gee_id = ( ! empty( $_GET[ 'gee_id' ] ) ) ? $_GET[ 'gee_id' ] : false;
...
$vc_bankaccholderpre= ( ! empty( $_GET[ 'vc_bankaccholderpre' ] ) ) ? $_GET[ 'vc_bankaccholderpre' ] : false;
$vc_bankaccholderstr= ( ! empty( $_GET[ 'vc_bankaccholderstr' ] ) ) ? $_GET[ 'vc_bankaccholderstr' ] : false;
$vc_bankaccholdernr= ( ! empty( $_GET[ 'vc_bankaccholdernr' ] ) ) ? $_GET[ 'vc_bankaccholdernr' ] : false;
$vc_bankaccholderzip= ( ! empty( $_GET[ 'vc_bankaccholderzip' ] ) ) ? $_GET[ 'vc_bankaccholderzip' ] : false;
$vc_bankaccholdercity= ( ! empty( $_GET[ 'vc_bankaccholdercity' ] ) ) ? $_GET[ 'vc_bankaccholdercity' ] : false;
$vc_check_sepa= ( ! empty( $_GET[ 'vc_check_sepa' ] ) ) ? $_GET[ 'vc_check_sepa' ] : false;
$vc_check_agb= ( ! empty( $_GET[ 'vc_check_agb' ] ) ) ? $_GET[ 'vc_check_agb' ] : false;
$vc_check_widerruf= ( ! empty( $_GET[ 'vc_check_widerruf' ] ) ) ? $_GET[ 'vc_check_widerruf' ] : false;
$vc_check_schufa= ( ! empty( $_GET[ 'vc_check_schufa' ] ) ) ? $_GET[ 'vc_check_schufa' ] : false;
$vc_check_optintel= ( ! empty( $_GET[ 'vc_check_optintel' ] ) ) ? $_GET[ 'vc_check_optintel' ] : false;
$vc_check_optinemail= ( ! empty( $_GET[ 'vc_check_optinemail' ] ) ) ? $_GET[ 'vc_check_optinemail' ] : false;
$vc_check_optinmail= ( ! empty( $_GET[ 'vc_check_optinmail' ] ) ) ? $_GET[ 'vc_check_optinmail' ] : false;
$gee_owner_name= ( ! empty( $_GET[ 'gee_owner_name' ] ) ) ? $_GET[ 'gee_owner_name' ] : false;
$gee_contact_company= ( ! empty( $_GET[ 'gee_contact_company' ] ) ) ? $_GET[ 'gee_contact_company' ] : false;
$gee_contact_name= ( ! empty( $_GET[ 'gee_contact_name' ] ) ) ? $_GET[ 'gee_contact_name' ] : false;
$gee_contact_str= ( ! empty( $_GET[ 'gee_contact_str' ] ) ) ? $_GET[ 'gee_contact_str' ] : false;
$gee_contact_zip= ( ! empty( $_GET[ 'gee_contact_zip' ] ) ) ? $_GET[ 'gee_contact_zip' ] : false;
$gee_contact_city= ( ! empty( $_GET[ 'gee_contact_city' ] ) ) ? $_GET[ 'gee_contact_city' ] : false;
$gee_contact_tel= ( ! empty( $_GET[ 'gee_contact_tel' ] ) ) ? $_GET[ 'gee_contact_tel' ] : false;
$gee_contact_email= ( ! empty( $_GET[ 'gee_contact_email' ] ) ) ? $_GET[ 'gee_contact_email' ] : false;
$gee_flur= ( ! empty( $_GET[ 'gee_flur' ] ) ) ? $_GET[ 'gee_flur' ] : false;
$vc_docname_contract= ( ! empty( $_GET[ 'vc_docname_contract' ] ) ) ? $_GET[ 'vc_docname_contract' ] : false;
$vc_docname_awa= ( ! empty( $_GET[ 'vc_docname_awa' ] ) ) ? $_GET[ 'vc_docname_awa' ] : false;
$vc_docname_gee= ( ! empty( $_GET[ 'vc_docname_gee' ] ) ) ? $_GET[ 'vc_docname_gee' ] : false;

$foreign_customer_id = ( ! empty( $_GET[ 'foreign_customer_id' ] ) ) ? $_GET[ 'foreign_customer_id' ] : false;
$vc_housetype= ( ! empty( $_GET[ 'vc_housetype' ] ) ) ? $_GET[ 'vc_housetype' ] : false;

$vc_tbname= ( ! empty( $_GET[ 'vc_tbname' ] ) ) ? $_GET[ 'vc_tbname' ] : false;
$vc_tbfirstname= ( ! empty( $_GET[ 'vc_tbfirstname' ] ) ) ? $_GET[ 'vc_tbfirstname' ] : false;
$vc_tbwiderspruch= ( ! empty( $_GET[ 'vc_tbwiderspruch' ] ) ) ? $_GET[ 'vc_tbwiderspruch' ] : false;


$vc_contact_email= ( ! empty( $_GET[ 'vc_contact_email' ] ) ) ? $_GET[ 'vc_contact_email' ] : false;
$vc_billingtype= ( ! empty( $_GET[ 'vc_billingtype' ] ) ) ? $_GET[ 'vc_billingtype' ] : false;
$btuserordertype_id= ( ! empty( $_GET[ 'btuserordertype_id' ] ) ) ? $_GET[ 'btuserordertype_id' ] : false;
$vc_contract_duration= ( ! empty( $_GET[ 'vc_contract_duration' ] ) ) ? $_GET[ 'vc_contract_duration' ] : false;
  ...
  if ($page==1)
  ...
  else if ($page==3)
    {
        echo("<h5>Order Management : &nbsp;&nbsp;<i>> <a href=index.php?page=3>Bestellformular</a> &nbsp; > <a href=index.php?page=4>Bestellungen - alle</a> &nbsp; > 
          ...
          <a href=index.php?page=4&action=orderprod32>P-Wechsel(32)</a> &nbsp; > 
          <a href=index.php?page=4&action=orderprod38>P-KüV(38)</a> &nbsp; > 
          <a href=index.php?page=4&action=orderprod39>P-KüB(39)</a> &nbsp; > 
          <a href=index.php?page=4&action=preorder20>Fehlende Daten(20)</a> &nbsp; > 
...
//      if ($location_id==6)
//        {
//        include("templates/checkformdata_obs.php");
//        }
//      else
//        {
...
//        }
    ...
    if (($ordererror==0)&&(($action=='vcorder')||($action=='vcupgradeconfirm')||($action=='reprov')))
      {
      echo("OS");
      $ordermode=1;
      if ($debug) echo("AVAILCHECK<br>");
    ...
    else if ($action=='vcupgrade')
      {
      require("templates/vcedit.php");
      }
      ...
      {
      require("templates/cancel.php");
      }
    else if ($action=='terminate')
      {
      require("templates/terminate.php");
      }
    else if ($action=='productchange')
      {
      require("templates/productchange.php");
      }
    else
      {
      //$action='vcorderform';
//      echo("FORM");
    ...
    else if ($action=='preorder20')
      {
      echo("<h3>> Vorbestellungen Fehlende Daten</h3><center>");
      }
    ...
    else if ($action=='orderprod32')
      {
      echo("<h3>> Bestellungen produktiv - Produktwechsel vorgemerkt</h3><center>");
      }
    else if ($action=='orderprod38')
      {
      echo("<h3>> Bestellungen produktiv - Kündigung vorgemerkt</h3><center>");
      }
    else if ($action=='orderprod39')
      {
      echo("<h3>> Bestellungen produktiv - Kündigung bestätigt</h3><center>");
      }
