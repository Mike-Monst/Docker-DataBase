...
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
// !!!! beim Einfügen einer neuen DB Rechte fuer crmweb1 setzen!!!!  sonst ist SEL unbestimmt
//print "SEL $select";
...
function add_vchistory($id)
  ...
  MYSQL_QUERY("insert into dslprov_vchistory (vc_id,vc_visprefnum,product_id,productadditional_id,productcap_id,vc_email,invoicemail_id,hardware_id,visp_id,vc_carelevel,vc_state,vc_state_voice,vc_state_billing,vc_statechangedate,vc_statechange_voice,vc_statechange_billing) select vc_id,vc_visprefnum,product_id,productadditional_id,productcap_id,vc_email,invoicemail_id,hardware_id,visp_id,vc_carelevel,vc_state,vc_state_voice,vc_state_billing,vc_statechangedate,vc_statechange_voice,vc_statechange_billing from dslprov_vc where vc_id='$id'");
  }
...
function get_isodate($serialtime)
 ...
 # $isodate=date("Y-m-d",$serialtime);
  $isodate=date("d.m.Y",$serialtime);
  return($isodate);  
  }


function get_isodate_f($serialtime)
  {
...
function get_isodate_ff($serialtime)
  {
...
function get_product($id)
  {
  $query="select prod_name_short from dslprov_product where prod_id='$id'";
  $result=mysql_query($query);
  $number=mysql_num_rows($result);
  if ($number!=1)
    {
  ...
  $ArticleTextRow=mysql_fetch_array($result);
  $value=$ArticleTextRow['prod_name_short'];
  return($value);
  }
...
function get_productcap($id)
  {
  $query="select btproduct_type,btproduct_bw from dslprov_productcap where btproduct_id='$id'";
  $result=mysql_query($query);
  $number=mysql_num_rows($result);
  if ($number!=1)
    {
  ...
  if ($value == 'ADSL') {$value='<b>ADSL!!!</b>';} 
  $value=$value.'<br>'.$ArticleTextRow['btproduct_bw'];
  return($value);
  }
...
function get_producta($id)
  {
  $query="select prod_name_short from dslprov_productadditional where prod_id='$id'";
  $result=mysql_query($query);
  $number=mysql_num_rows($result);
  if ($number!=1)
    {
...
function get_hardware($id)
  {
  $query="select hardware_value_short from dslprov_options_hardware where hardware_id='$id'";
  $result=mysql_query($query);
  $number=mysql_num_rows($result);
  if ($number!=1)
    {
  ...
  $ArticleTextRow=mysql_fetch_array($result);
  $value=$ArticleTextRow['hardware_value_short'];
  return($value);
  }
